...
func init() {
	android.RegisterModuleType("lumine_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lumineExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lumineExpandVariables(ctx, dep_file)
	...
	cmd := lumineExpandVariables(ctx, String(g.properties.Cmd))
