...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^lumine_") ; then
        LUMINE_BUILD=$(echo -n $1 | sed -e 's/^lumine_//g')
    else
        LUMINE_BUILD=
    fi
    export LUMINE_BUILD
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Lumine model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch lumine_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/LumineDroid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop org.luminedroid.device | grep -q "$LUMINE_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $LUMINE_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.luminedroid.device | grep -q "$LUMINE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.luminedroid.device | grep -q "$LUMINE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function build_kernel() {
    local lumine_version="LumineDroid-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${lumine_version}")
        if [ -n "${LUMINE_MIRROR}" ]; then
            repo_init_args+=("--reference" "${LUMINE_MIRROR}")
        fi
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/LumineDroid/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
