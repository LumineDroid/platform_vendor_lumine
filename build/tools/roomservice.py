...
lumine_local_manifest = ".repo/local_manifests/roomservice.xml"
lumine_default_revision =  "blumea"
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = lumine_default_revision
...
def detect_revision(repo):
    ...
    print("Calculated revision: %s" % lumine_default_revision)

    if has_branch(result, lumine_default_revision):
        return lumine_default_revision

    print("Branch %s not found" % lumine_default_revision)
    sys.exit()
