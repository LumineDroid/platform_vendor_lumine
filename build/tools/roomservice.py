...
lumine_local_manifest = ".repo/local_manifests/local_manifests.xml"
lumine_default_revision = "bynx"
lumine_default_revision =  os.getenv('ROOMSERVICE_DEFAULT_BRANCH', 'bynx')
lumine_dependencies = "lumine.dependencies"
org_manifest = "lumine-devices"  # leave empty if org is provided in manifest
org_display = "LumineDroid-Devices"  # needed for displaying
...
def get_revision(manifest=None, p="build"):
    return lumine_default_revision

def get_from_manifest(device_name):
    if os.path.exists(lumine_local_manifest):
        man = load_manifest(lumine_local_manifest)
...
def is_in_manifest(project_path):
    for local_path in load_manifest(lumine_local_manifest).findall("project"):
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(lumine_local_manifest)
    ...
    for repo in repos:
        ...
        if 'branch' in repo:
            ...
            repo_branch = lumine_default_revision
    ...
    f = open(lumine_local_manifest, 'w')
    f.write(raw_xml)
    f.close()
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, lumine_dependencies))
    ...
    for dependency in dependencies:
        if not is_in_manifest(dependency['target_path']):
            if not dependency.get('branch'):
                dependency['branch'] = (
                    fallback_branch or get_revision() or lumine_default_revision
                )
...
def detect_revision(repo):
    ...
    if has_branch(result, lumine_default_revision):
        print("Falling back to custom revision: %s"
              % lumine_default_revision)
        return lumine_default_revision
